...
- (void)handleMethodCall:(FlutterMethodCall *)call
  ...
  if ([@"getDeviceInfo" isEqualToString:call.method]) {
    ...
    // Fix: Check for isiOSAppOnVision selector at runtime, as it's only available in visionOS 26.1+
    // The @available(iOS 17.0, *) check was incorrect
    SEL selector = NSSelectorFromString(@"isiOSAppOnVision");
    if (selector && [[info class] instancesRespondToSelector:selector]) {
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
      isiOSAppOnVision = [NSNumber numberWithBool:(BOOL)[info performSelector:selector]];
